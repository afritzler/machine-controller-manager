...
func (c *controller) inPlaceUpdate(ctx context.Context, d driver.Driver, machine *v1alpha1.Machine, secretData map[string][]byte) (machineutils.RetryPeriod, error) {
	...
	if cond.Reason == v1alpha1.DrainSuccessful {
		...
		newMachineClass, err := c.getNewMachineClassForMachine(machine)
		if err != nil {
			return machineutils.ShortRetry, err
		}

		if newMachineClass != nil {
			// Attempt to update the machine's infrastructure resource
			_, err = d.UpdateMachine(ctx, &driver.UpdateMachineRequest{
				Machine:      machine,
				MachineClass: newMachineClass,
